...
import { PrimaryButton, TextButton, CustomDialog } from '../../../core/component';
...
export const HomePage = ({ setIsOpenAccMenu, isShortcutKeys }: HomePageProps) => {
  ...
  const [isWaitingNextWord, setIsWaitingNextWord] = useState(0);
  ...
  useEffect(() => {
    ...
    checkedLibraries.forEach((libraryTitle: string) => {
      const library = libraries.find((lib: LibraryModel) => lib.title === libraryTitle);
      if (library) {
        newWords.push(
            ...
            switch (level) {
              ...
              case 100:
                return onlyThisLevel
                  ? word.level === 'C2'
                  : word.level === 'C2' || word.level === 'C1' || word.level === 'B2' || word.level === 'B1' || word.level === 'A';
            }
          }),
        );
      }
  ...
  const handleCheckAnswer = async () => {
    if (!currentWord) return;
    if (pageState === 'translate_passage') {
      if (isWaitingNextWord > 1) {
        setNotification('Một đoạn văn chỉ được trả lời 2 lần', 'info');
        return;
      }
      if (answer) {
        let rawScore = await getTranslationScore(currentPassage, answer);
        let scoreStr = rawScore.toString();
        let score;

        if (scoreStr.length > 10) {
          // Lấy 2 ký tự số đầu tiên từ chuỗi
          const match = scoreStr.match(/\d{2}/); // tìm 2 chữ số liên tiếp đầu tiên
          score = match ? parseInt(match[0]) : 0; // fallback nếu không tìm thấy
        } else {
          score = parseInt(scoreStr);
        }
        setScore(score.toString());
        setIsWaitingNextWord(isWaitingNextWord + 1);
      }
      return;
    }
    let answerToCheck = '';
  ...
  const handleSpeak = async () => {
    ...
    if (currentAudio) {
      ...
      if ('speechSynthesis' in window) {
        ...
        const englishVoice =
          voices.find((voice) => voice.lang.startsWith('en') && voice.name.includes('Female')) || voices.find((voice) => voice.lang.startsWith('en'));
    ...
    >
      ...
      <Stack direction={'row'} className="justify-between items-start p-3 pt-[95px]" spacing={2} sx={{ height: isMobile ? 'auto' : '100vh' }}>
        ...
        <Stack
          spacing={2}
          flex={isMobile ? 1 : 2}
          className={`p-4 rounded-3xl min-h-full items-center`}
        ...
        >
          ...
          {wordList.length < 1 && (
            <i style={{ color: theme.palette.text.secondary, fontSize: '14px', textAlign: 'center' }}>
              Vui lòng chọn bộ câu hỏi trong thư viện để thực hiện !
            </i>
          )}
          <Box className="relative w-full flex items-center justify-center">
            ...
            />
            <HighlightOutlined
              className={`transform rotate-[-135deg] rounded-full animate-bounce absolute top-[-10px] ${
                pageState === 'translate_passage' ? 'right-[5%]' : 'right-[20%]'
          ...
          <Stack spacing={2} className={`${isMobile ? 'px-4' : 'px-16'} w-full flex`} direction={'column'}>
            <Stack direction={isMobile ? 'column' : 'row'} spacing={2} alignItems="center" className="flex w-full">
              <PrimaryButton
                ...
                title={pageState === 'translate_passage' ? t('next_word') : t('answer_result')}
                handleClick={pageState === 'translate_passage' ? handleNextWord : handleShowResult}
                bgColor="#29a329"
            ...
            <Stack direction={isMobile ? 'column' : 'row'} spacing={2} justifyContent="center" alignItems="center" width={'100%'}>
              <PrimaryButton
                ...
                bgColor="#ffb31a"
                tooltip={pageState != 'translate' ? undefined : 'Ctrl'}
              />
              <PrimaryButton
                width={isMobile ? '100%' : '40%'}
          ...
          {pageState != 'translate_passage' ? (
            <Typography mt={2} color="gray">
              {t('correct')}: <b style={{ color: 'lightgreen' }}>{correctCount}</b> | {t('incorrect')}:{' '}
              <b style={{ color: 'salmon' }}>{wrongCount}</b>
            </Typography>
          ) : (
