...
export const LibraryDetail = ({ setOpenDialog }: TopicDetailProps) => {
  ...
  return (
    <Stack className="text-black" spacing={2}>
      ...
      <Stack component={'header'} direction={'row'} justifyContent={'space-between'} alignItems={'center'}>
        <h1 className="font-bold text-[16px] pl-3">Library Detail</h1>
        <TextButton startIcon={<Clear />} width={'35px'} fontSize={23} handleClick={() => setOpenDialog(false)} color="red" />
      </Stack>
      {/* Info */}
      <Stack direction={'row'} flex={1}>
        {/* Avatar */}
        <Stack direction="column" spacing={1} className=" relative items-center">
          ...
          {/* IsPublic */}
          <OutlineButton title="Public" handleClick={() => {}} height={'30px'} width={'40px'} />
        </Stack>
        <Stack spacing={1} direction={'column'} boxShadow={2} className="flex-1 bg-[#e8e8e8] rounded-xl mt-2 mx-6 py-4 self-center">
          ...
          <Stack direction={'row'} spacing={8} alignItems={'center'} className="px-4 " justifyContent={'space-between'}>
            <span className="text-[14px] text-black font-semibold ">Library Name</span>
          ...
          {/* Created By */}
          <Stack direction={'row'} spacing={8} alignItems={'center'} className="px-4 h-[30px]" justifyContent={'space-between'}>
            <span className="text-[14px] text-black font-semibold flex-1 ">Created User</span>
            <span className="text-[14px] text-black font-semibold flex-1 text-center">Huỳnh Ngọc Triều</span>
          </Stack>
        </Stack>
      </Stack>
      {/* Buttons */}
      <Stack direction={'row'} spacing={2} className="mx-6" justifyContent={'center'}>
        <OutlineButton
          ...
          height={'30px'}
        />
        <OutlineButton title="Delete Library" handleClick={() => {}} width={'160px'} height={'30px'} bgColor="red" />
      </Stack>
      {/* Add new word */}
      {onAddNewWord && (
